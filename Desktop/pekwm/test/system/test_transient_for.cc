...
public:
	...
	virtual void handleFd(int fd)
	{
		...
		if (_loop) {
			...
			std::cout << "PROGRESS: transient " << _t_win2
				  << " set to " << _t_win1 << std::endl;
			XSetTransientForHint(X11::getDpy(), _window, _t_win2);
			std::cout << "PROGRESS: transient " << _t_win2
				  << " set to " << _window << std::endl;
			_loop = false;
		} else if (_main_first && _window != None) {
			// test destroying the window that the transient window
			// is transient for before destroying the transient
			// window.
			std::cout << "PROGRESS: destroy main window"
				  << std::endl;
			XDestroyWindow(X11::getDpy(), _window);
			_window = None;
		} else if (_t_win1 != None) {
			std::cout << "PROGRESS: destroy transient window 1"
				  << std::endl;
			XDestroyWindow(X11::getDpy(), _t_win1);
			_t_win1 = None;
		} else if (_t_win2 != None) {
			std::cout << "PROGRESS: destroy transient window 2"
				  << std::endl;
