...
TestConfig::testParseMenuActionGotoItem()
{
	...
	ASSERT_EQUAL("action set", ACTION_TYPE_MENU_GOTO, action.getAction());
	...
	ASSERT_EQUAL("action set", ACTION_TYPE_NO, action.getAction());
}
...
TestConfig::testParseMenuActionInvalid()
{
	...
	ASSERT_EQUAL("action not set", ACTION_TYPE_NO, action.getAction());
}
...
TestConfig::testLoadDebug()
{
	...
	ASSERT_EQUAL("default level", getDebugLevel(), DEBUG_LEVEL_TRACE);
	...
	ASSERT_EQUAL("default level", getDebugLevel(), DEBUG_LEVEL_DEBUG);
}
