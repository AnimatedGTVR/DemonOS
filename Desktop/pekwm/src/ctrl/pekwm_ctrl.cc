...
static bool focusClient(Window win, NetWmSource source, long server_time)
{
	long data[3] = {};
	data[0] = static_cast<long>(source);
	if (source != NET_WM_SOURCE_LEGACY) {
		data[1] = server_time == -1
			? X11::queryTime(true)
			: server_time;
	}
...
int main(int argc, char* argv[])
{
	...
	NetWmSource source = NET_WM_SOURCE_PAGER;
	long server_time = -1;
	while ((ch = getopt_long(argc, argv, "a:c:C:d:g:hsS:T:w:W:",
				 opts, nullptr))
	       != -1) {
		switch (ch) {
		case 'a':
		...
		case 's':
			...
			break;
		case 'S':
			source = pekwm::str_to_net_wm_source(optarg);
			break;
		case 'T':
			server_time = std::stoi(optarg);
			break;
		case 'h':
	...
	case PEKWM_CTRL_ACTION_FOCUS:
		...
		res = focusClient(client, source, server_time);
		printRes(res);
		break;
