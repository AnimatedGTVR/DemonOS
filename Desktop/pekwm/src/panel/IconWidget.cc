	  ...
	  _icon_scaled(nullptr)
{
	...
	pekwm::observerMapping()->addObserver(&_var_data, this, 50);
	load();
}
...
uint
IconWidget::getRequiredSize(uint max)
{
	uint size;
	if (_scale || _icon == nullptr) {
		size = _theme.getHeight();
	} else {
		size = _icon->getWidth() + 2;
	}
	return max > 0 ? std::min(max, size) : size;
}
...
IconWidget::parseIcon(const CfgParser::Entry* section)
{
	...
	keys.add_bool("LOWERCASE", _lowercase);
