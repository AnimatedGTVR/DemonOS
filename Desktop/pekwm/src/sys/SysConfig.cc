...
SysConfig::SysConfig(const std::string &config_file, Os *os)
	: _config_file(config_file),
	  ...
	  _latitude(NAN),
	  _longitude(NAN),
	  _screen_lock_idle_time_s(0),
	  _screen_lock_allow_inhibit(true),
	  _screen_lock_command("pekwm_lock"),
...
SysConfig::SysConfig(const SysConfig &cfg)
	: _config_file(cfg._config_file),
	  ...
	  _screen_lock_idle_time_s(cfg._screen_lock_idle_time_s),
	  _screen_lock_allow_inhibit(cfg._screen_lock_allow_inhibit),
	  _screen_lock_command(cfg._screen_lock_command),
...
SysConfig::parseConfig()
{
	...
	keys.add_numeric<uint>("SCREENLOCKIDLETIME", _screen_lock_idle_time_s,
			       0);
	keys.add_bool("SCREENLOCKALLOWINHIBIT", _screen_lock_allow_inhibit,
		      true);
	keys.add_string("SCREENLOCKCOMMAND", _screen_lock_command,
			"pekwm_lock");
