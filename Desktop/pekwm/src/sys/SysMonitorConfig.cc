...
std::string
SysMonitorConfig::mkId(bool includeE)
{
...
SysMonitorConfig::find(MonitorsConfig &monitors)
{
	int includeE = 1;
	do {
		std::string id = mkId(static_cast<bool>(includeE));
		if (id.empty()) {
			return false;
		}
		std::map<std::string, MonitorsConfig>::iterator
			it(_config.find(id));
		if (it != _config.end()) {
			monitors = it->second;
			return true;
	...
	P_TRACE("did not find a configuration for connected outputs "
		" looking in " << _config.size() << " configurations");
	return false;
}

void
SysMonitorConfig::disableUnused(const MonitorsConfig &monitors,
				X11_XRandr_ScreenResources *sr)
{
	for (int i = 0; i < sr->size(); ++i) {
		X11_XRandr_OutputInfo *oi = sr->getOutput(i);
		if (oi->isConnected()
		    && oi->isCrtcUsed()
		    && !monitors.hasMonitor(oi->getName())) {
			P_TRACE("disabling unused output " << oi->getName());
			sr->disableOutput(oi);
		}
	}
}
...
SysMonitorConfig::apply(const MonitorsConfig &monitors)
{
	...
	// Disable all connected outputs not in the configuration before
	// applying to free up CRTC resources.
	disableUnused(monitors, *sr);

	// Read screen resources after disabling outputs, ensuring all free
	// CRTCs are available when applying configuration.
	sr = mkX11_XRandr_ScreenResouces();
	if (sr->empty()) {
		return false;
	}
	...
	// Disabling outputs before enabling new ones can fail, try again
	// after if any disabling failed.
	disableUnused(monitors, *sr);
