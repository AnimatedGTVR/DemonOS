...
public:
	...
	uint getScreenLockIdleTimeS() const { return _screen_lock_idle_time_s; }
	void setScreenLockIdleTimeS(uint idle_time_s) {
		_screen_lock_idle_time_s = idle_time_s;
	}
	bool isScreenLockAllowInhibit() const {
		return _screen_lock_allow_inhibit;
	}
	const std::string &getScreenLockCommand() const {
		return _screen_lock_command;
	}
...
private:
	...
	/* Seconds of idle time before screen lock is started. */
	uint _screen_lock_idle_time_s;
	/* If true, honor to idle inhibit messages. */
	bool _screen_lock_allow_inhibit;
	/* Command to execute when starting the screen locker. */
	std::string _screen_lock_command;
