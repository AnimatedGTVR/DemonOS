...
public:
	...
	virtual bool isSet(enum Type type) {
		if (type == OS_SELECT_READ) {
			return memcmp(&_rfds, &_empty_set, sizeof(fd_set));
		} else if (type == OS_SELECT_WRITE) {
			return memcmp(&_wfds, &_empty_set, sizeof(fd_set));
		} else {
			return memcmp(&_rfds, &_empty_set, sizeof(fd_set))
				|| memcmp(&_wfds, &_empty_set, sizeof(fd_set));
		}
	}
	...
	virtual bool clearIfSet(int fd, enum Type type)
	{
		if (isSet(fd, type)) {
			FD_CLR(fd, type == OS_SELECT_READ ? &_rfds : &_wfds);
			return true;
		}
		return false;
	}
...
public:
	...
	/**
	 * Wrapper for getpwent getting the login for the current user.
	 */
	...
	/**
	 * Wrapper for gethostname getting the name of the running host.
	 */
	virtual std::string getHostname()
	{
		char hostname[HOST_NAME_MAX + 1] = {};
		gethostname(hostname, sizeof(hostname));
		// NUL termination on truncation (and return value) varies
		// between platforms, better safe than sorry.
