...
public:
	...
	virtual bool canUseCrtc(RRCrtc crtc) const {
		if (_oi != nullptr) {
			for (int i = 0; i < _oi->ncrtc; ++i) {
				if (_oi->crtcs[i] == crtc) {
					return true;
				}
			}
	...
	virtual bool supportsMode(RRMode mode) const {
		if (_oi != nullptr) {
			for (int i = 0; i < _oi->nmode; ++i) {
				if (_oi->modes[i] == mode) {
					return true;
				}
			}
...
public:
	...
	virtual bool disableOutput(X11_XRandr_OutputInfo *oi) {
		return disableOutput(oi->getCrtcId());
	}

	virtual bool disableOutput(RRCrtc crtc) {
		if (crtc == None) {
			return true;
		}

		Status status =
			XRRSetCrtcConfig(X11::getDpy(), _sr, crtc,
					 CurrentTime, 0, 0, None, RR_Rotate_0,
					 nullptr, 0);
		if (status != RRSetConfigSuccess) {
			P_WARN("failed to disable output: "
			       << errString(status));
		}
	...
	virtual bool setCrtcConfig(X11_XRandr_OutputInfo *oi, int x, int y,
		...
		RRMode mode = mi->getId();
		if (! oi->supportsMode(mode)) {
			P_WARN("output " << oi->getName() 
			       << " does not support mode " << mode);
			return false;
		}

		// Re-use existing CRTC if compatible
		RRCrtc crtc = None;
		RRCrtc current = oi->getCrtcId();
		if (current != None) {
			Destruct<XRRCrtcInfo> ci(
				XRRGetCrtcInfo(X11::getDpy(), _sr, current),
				XRRFreeCrtcInfo);
			if (*ci
			    && ci->rotations & rotation
			    && oi->canUseCrtc(current)) {
				crtc = current;
			}
		}

		if (crtc == None) {
			crtc = findFreeCrtcForOutput(oi, rotation);
		}

		if (crtc == None) {
			P_WARN("no compatible CRTC found for "
			       << oi->getName());
			return false;
		}
...
private:
	...
	RRCrtc findFreeCrtcForOutput(X11_XRandr_OutputInfo *oi,
				     Rotation rotation) {
		for (int i = 0; i < _sr->ncrtc; ++i) {
			RRCrtc crtc = _sr->crtcs[i];
			Destruct<XRRCrtcInfo> ci(
				XRRGetCrtcInfo(X11::getDpy(), _sr, crtc),
				XRRFreeCrtcInfo);
			if (*ci != nullptr
			    && ci->noutput == 0
		    && (ci->rotations & rotation)
...
public:
	...
	virtual bool disableOutput(X11_XRandr_OutputInfo *) {
		return false;
	}
	virtual bool disableOutput(RRCrtc) {
		return false;
	}
