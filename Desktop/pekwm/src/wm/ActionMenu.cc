	  ...
	  _has_dynamic(false)
{
	...
	if (_menu_type == MENU_TYPE_WINDOW) {
		_action_ok = WINDOWMENU_OK;
	} else if (_menu_type == MENU_TYPE_ROOT
		   || _menu_type == MENU_TYPE_ROOT_STANDALONE) {
...
void
ActionMenu::insert(const std::string &name, bool parse_name,
		   PWinObj *wo_ref, PTexture *icon)
{
	WORefMenu::insert(name, parse_name, wo_ref, icon);
}

void
ActionMenu::insert(const std::string &name, bool parse_name,
		   const ActionEvent &ae, PWinObj *wo_ref, PTexture *icon)
{
	WORefMenu::insert(name, parse_name, ae, wo_ref, icon);
}
...
ActionMenu::parseItem(CfgParser::Entry* section, PMenu::Item* parent)
{
	...
	if (value && ActionConfig::parseActions(value->getValue(), ae,
		...
		if (ae.isOnlyAction(ACTION_TYPE_MENU_DYN)) {
			_has_dynamic = true;
...
ActionMenu::rebuildDynamic(void)
{
	...
	for (; it != m_end(); ++it) {
		if ((*it)->getAE().isOnlyAction(ACTION_TYPE_MENU_DYN)) {
