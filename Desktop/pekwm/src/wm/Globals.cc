...
namespace pekwm
{
	...
	bool enumNameToNames(const std::string &name,
			     std::vector<const char*> &names)
	{
		return lib_enum_name_to_names(name.c_str(), names)
			|| tk_enum_name_to_names(name.c_str(), names)
			|| wm_enum_name_to_names(name.c_str(), names);
	}
}
