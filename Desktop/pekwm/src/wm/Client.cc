			    ...
			    AutoProperty *prop)
{
	...
	if (is_new && _parent->isMapped()) {
		initConfig.focus = initFocus(prop);
	}
}

bool
Client::initFocus(AutoProperty *prop)
{
	bool focus = pekwm::config()->isFocusNew();
	// focus client, even if not focusing new clients, if it is transient
	// for the currently focused window.
	if (! focus && _transient_for) {
		focus = _transient_for->isFocused()
			&& pekwm::config()->isFocusNewChild();
	}

	Cardinal init_time = 0;
	if (X11::getCardinal(_window, NET_WM_USER_TIME, init_time)
	    && init_time == 0) {
		// Setting _NET_WM_USER_TIME to 0 indicates that the window
		// should not be given input focus.
		P_TRACE(_window << " _NET_WM_USER_TIME set to 0, not giving "
			"input focus on create");
		focus = false;
	} else if (init_time != 0
		   && pekwm::config()->isFocusStealProtect()) {
		Window focus_win = PWinObj::getFocusedWindow();
		Cardinal focus_time;
		if (X11::getCardinal(focus_win, NET_WM_USER_TIME, focus_time)
		    && init_time < focus_time) {
			P_TRACE(_window << " _NET_WM_USER_TIME set to " <<
				init_time << ", less than focused window " <<
				focus_win << " time " << focus_time <<
				", not giving input focus on create");
			focus = false;
		}
	}

	// overwrite focus if set in the auto property
	if (prop && prop->isMask(PROPERTY_TYPE_FOCUS_NEW)) {
		P_TRACE(_window << " overriding focus " << focus << " with "
			" property focus " << prop->focus_new);
		focus = prop->focus_new;
	}
	return focus;
}
...
Client::applyAutoprops(AutoProperty *ap)
{
	...
	if (ap->isMask(PROPERTY_TYPE_STICKY)) {
		_sticky = ap->sticky;
	}
	if (ap->isMask(PROPERTY_TYPE_SHADED)) {
		_state.shaded = ap->shaded;
	}
	if (ap->isMask(PROPERTY_TYPE_MAXIMIZED_VERTICAL)) {
		_state.maximized_vert = ap->maximized_vertical;
	}
	if (ap->isMask(PROPERTY_TYPE_MAXIMIZED_HORIZONTAL)) {
		_state.maximized_horz = ap->maximized_horizontal;
	}
	if (ap->isMask(PROPERTY_TYPE_FULLSCREEN)) {
		_state.fullscreen = ap->fullscreen;
	}
	if (ap->isMask(PROPERTY_TYPE_ICONIFIED)) {
		_iconified = ap->iconified;
	}
	if (ap->isMask(PROPERTY_TYPE_TITLEBAR)) {
		setTitlebar(ap->titlebar);
	}
	if (ap->isMask(PROPERTY_TYPE_BORDER)) {
		setBorder(ap->border);
	}
	if (ap->isMask(PROPERTY_TYPE_LAYER) && (ap->layer <= LAYER_MENU)) {
		setLayer(ap->layer);
	}
	if (ap->isMask(PROPERTY_TYPE_SKIP)) {
		_state.skip = ap->skip;
	}
	if (ap->isMask(PROPERTY_TYPE_FOCUSABLE)) {
		_focusable = ap->focusable;
	}
	if (ap->isMask(PROPERTY_TYPE_WORKSPACE)) {
		_workspace = ap->workspace;
	}
	if (ap->isMask(PROPERTY_TYPE_CFG_DENY)) {
		_state.cfg_deny = ap->cfg_deny;
	}
	if (ap->isMask(PROPERTY_TYPE_ALLOWED_ACTIONS)) {
		applyActionAccessMask(ap->allowed_actions, true);
	}
	if (ap->isMask(PROPERTY_TYPE_DISALLOWED_ACTIONS)) {
		applyActionAccessMask(ap->disallowed_actions, false);
	}
	if (ap->isMask(PROPERTY_TYPE_OPACITY)) {
		setOpacity(ap->focus_opacity, ap->unfocus_opacity);
	}
	if (ap->isMask(PROPERTY_TYPE_ICON)) {
...
Client::applyActionAccessMask(uint mask, bool value)
{
	...
	if (mask & ACTION_ACCESS_MAXIMIZE_HORIZONTAL) {
		_actions.maximize_horz = value;
	}
	if (mask & ACTION_ACCESS_MAXIMIZE_VERTICAL) {
		_actions.maximize_vert = value;
	}
	if (mask & ACTION_ACCESS_FULLSCREEN) {
		_actions.fullscreen = value;
	}
	if (mask & ACTION_ACCESS_SET_WORKSPACE) {
...
Client::getAPDecorName()
{
	...
	if (ap && ap->isMask(PROPERTY_TYPE_DECOR)) {
