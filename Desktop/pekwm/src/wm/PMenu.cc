...
ActionEvent _ae_select_item =
	ActionEvent(2, ACTION_TYPE_MENU_GOTO, ACTION_TYPE_MENU_SELECT);
...
PMenu::handleButtonPress(XButtonEvent *ev)
{
	if (*_menu_wo == ev->window) {
		handleItemEvent(MOUSE_EVENT_BUTTON_PRESS, ev->x, ev->y);
 ...
 */
void
PMenu::handleItemEvent(MouseEvent type, int x, int y)
{
