...
{
	int type_i = 1;
	while (type_i != static_cast<int>(WIN_LAYOUTER_TYPE_NO)) {
...
Workspaces::showWorkspaceIndicator(void)
{
	...
	TimeoutAction action(ACTION_TYPE_HIDE_WORKSPACE_INDICATOR, timeout);
...
Workspaces::gotoWorkspace(uint direction, bool focus, bool warp)
{
	...
	case WORKSPACE_CHANGE_PREV:
		...
		if (_active > row_min) {
			workspace = _active - 1;
		} else if (direction == WORKSPACE_CHANGE_PREV) {
			workspace = row_max;
		} else {
			switched = false;
		}
		break;
	case WORKSPACE_CHANGE_PREV_N:
	case WORKSPACE_CHANGE_LEFT_N:
		dir = 1;

		if (_active > 0) {
			workspace = _active - 1;
		} else if (direction == WORKSPACE_CHANGE_PREV_N) {
	...
	case WORKSPACE_CHANGE_RIGHT:
		...
		if (_active < row_max) {
			workspace = _active + 1;
		} else if (direction == WORKSPACE_CHANGE_NEXT) {
			workspace = row_min;
		} else {
			switched = false;
		}
		break;
	case WORKSPACE_CHANGE_NEXT_N:
	case WORKSPACE_CHANGE_RIGHT_N:
		dir = 2;

		if (_active < (_workspaces.size() - 1)) {
			workspace = _active + 1;
		} else if (direction == WORKSPACE_CHANGE_NEXT_N) {
			workspace = 0;
		} else {
			switched = false;
		}
	...
	case WORKSPACE_CHANGE_UP:
		...
		} else if (direction == WORKSPACE_CHANGE_PREV_V) {
			// Bottom left
	...
	case WORKSPACE_CHANGE_DOWN:
		...
		if (_active + per_row < _workspaces.size()) {
			workspace = _active + per_row;
		} else if (direction == WORKSPACE_CHANGE_NEXT_V) {
			workspace = _active - cur_row * per_row;
		} else {
			switched = false;
		}
 ...
 */
PWinObj*
Workspaces::findDirectional(PWinObj *wo, Direction dir, uint skip)
{
	...
	for (; it != _wobjs.end(); ++it) {
		...
		if (! (*it)->isType(PWinObj::WO_FRAME) ||
			...
			continue; // skip frame with skip
		}

		uint overlap = overlapPercent((*it));
		if (overlap > 90) {
			continue; // mostly hidden window, skip it
		}
