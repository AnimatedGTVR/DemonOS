...
public:
	ClientState()
		: maximized_vert(false),
		  ...
		  skip(0),
		  decor(DECOR_STATE_TITLEBAR|DECOR_STATE_BORDER),
...
public: // Public Member Functions
	...
	bool hasTitlebar(void) const {
		return _state.decor & DECOR_STATE_TITLEBAR;
	}
	bool hasBorder(void) const {
		return _state.decor & DECOR_STATE_BORDER;
	}
	...
	inline void setTitlebar(bool titlebar) {
		if (titlebar) {
			_state.decor |= DECOR_STATE_TITLEBAR;
		} else {
			_state.decor &= ~DECOR_STATE_TITLEBAR;
		}
	}
	...
	inline void setBorder(bool border) {
		if (border) {
			_state.decor |= DECOR_STATE_BORDER;
		} else {
			_state.decor &= ~DECOR_STATE_BORDER;
		}
	}
...
private:
	...
	void setClientInitConfig(ClientInitConfig &initConfig, bool is_new,
				 AutoProperty *prop);
	bool initFocus(AutoProperty *prop);
