...
static Util::StringTo<ActionType> menu_action_map[] = // subset of enum
	{{"NEXTITEM", ACTION_TYPE_MENU_NEXT},
	 {"PREVITEM", ACTION_TYPE_MENU_PREV},
	 {"GOTOITEM", ACTION_TYPE_MENU_GOTO},
	 {"SELECT", ACTION_TYPE_MENU_SELECT},
	 {"ENTERSUBMENU", ACTION_TYPE_MENU_ENTER_SUBMENU},
	 {"LEAVESUBMENU", ACTION_TYPE_MENU_LEAVE_SUBMENU},
...
Config::Config() :
	_debug_file("/dev/null"), _debug_level(DEBUG_LEVEL_WARNING),
	...
	_harbour_placement(HARBOUR_PLACEMENT_TOP),
	_harbour_orientation(ORIENTATION_TOP_TO_BOTTOM),
	_harbour_head_nr(0),
	...
	_sys_enabled(true)
{
	...
	for (uint i = 0; i <= MOUSE_ACTION_LIST_NO; i++) {
		MouseActionList mal = static_cast<MouseActionList>(i);
		_mouse_action_map[mal] = new std::vector<ActionEvent>();
	}
}
...
Config::loadScreen(CfgParser::Entry *section)
{
	...
	_screen_curr_head_selector = pekwm::str_to_curr_head_selector(
			curr_head_selector);
	_screen_on_close_focus_raise = pekwm::str_to_on_close_focus_raise(
...
Config::loadHarbour(CfgParser::Entry *section)
{
	...
	_harbour_placement = pekwm::str_to_harbour_placement(value_placement);
	_harbour_orientation = pekwm::str_to_orientation(value_orientation);
	if (_harbour_placement == HARBOUR_PLACEMENT_NO) {
		_harbour_placement = HARBOUR_PLACEMENT_RIGHT;
	}
	if (_harbour_orientation == ORIENTATION_NO) {
		_harbour_orientation = ORIENTATION_TOP_TO_BOTTOM;
	}
...
Config::loadDebug(CfgParser::Entry *section)
{
	...
	_debug_level = pekwm::str_to_debug_level(debug_level_str);
...
bool
Config::parseActionAccess(const std::string &action_mask, uint &mask)
{
	...
	if (Util::splitString(action_mask, tok, " \t")) {
		...
		for (; it != tok.end(); ++it) {
			mask |= pekwm::str_to_action_access(*it);
		}
	}
...
Config::parseMoveResizeAction(const std::string &action_string, Action &action)
{
	...
	action.setAction(pekwm::str_to_move_resize(tok[0]));
	if (action.getAction() == MOVE_RESIZE_NO) {
	...
	if (tok.size() == 2) {
		...
		case MOVE_RESIZE_MOVE_HORIZONTAL:
		case MOVE_RESIZE_MOVE_VERTICAL:
		case MOVE_RESIZE_RESIZE_HORIZONTAL:
		case MOVE_RESIZE_RESIZE_VERTICAL: {
			char *endptr = nullptr;
			action.setParamI(0,
...
{
	action.setAction(pekwm::str_to_input(val));
...
Config::getMenuMask(const std::string &mask)
{
	...
	std::vector<std::string>::iterator it = tok.begin();
	for (; it != tok.end(); ++it) {
		uint val = pekwm::str_to_mouse_event(*it);
...
Config::parseMenuAction(const std::string &action_string, Action &action)
{
	...
	if (Util::splitString(action_string, tok, " \t", 2)) {
		...
		if (action.getAction() == ACTION_TYPE_MENU_GOTO) {
			int idx = stoi_safe(tok[1], -1);
			if (idx > 0) {
				action.setParamI(0, idx - 1);
			} else {
				action.setAction(ACTION_TYPE_NO);
			}
		}
	}

	return action.getAction() != ACTION_TYPE_NO;
}
...
Config::loadMouseConfig(const std::string &mouse_file)
{
	...
	if (section) {
		...
		for (; edge_it != section->end(); ++edge_it)  {
			uint pos = pekwm::str_to_direction(
					(*edge_it)->getName());
		     ...
		     ActionOk action_ok)
{
	...
	for (; it != section->end(); ++it) {
		...
		ae.type = pekwm::str_to_mouse_event((*it)->getName());
