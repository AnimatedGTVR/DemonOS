...
public:
	...
	void place(void);
	void render();
	virtual void callback(std::vector<std::string> value);
...
PekwmPanel::PekwmPanel(const PanelConfig &cfg, PanelTheme &theme,
	...
	: X11App(Geometry(sh->x, sh->y,
		 ...
		 "", "panel", "pekwm_panel",
		 WINDOW_TYPE_DOCK, 0, nullptr, sh, true),
	  _os(mkOs()),
#ifdef PEKWM_HAVE_DBUS
	  _dbus(nullptr),
#endif // PEKWM_HAVE_DBUS
...
#ifdef PEKWM_HAVE_DBUS
	_dbus = new Dbus(getSelect());
	_dbus->matchAdd(&_dbus_mpris);
	_dbus->listNames(this);
#endif // PEKWM_HAVE_DBUS
}
...
void
PekwmPanel::callback(std::vector<std::string> value)
{
#ifdef PEKWM_HAVE_DBUS
	std::vector<std::string>::iterator it(value.begin());
	for (; it != value.end(); ++it) {
		if (pekwm::str_starts_with(*it, "org.mpris.MediaPlayer2.")) {
			_dbus->call(&_dbus_mpris, "Status:" + *it);
			_dbus->call(&_dbus_mpris, "Get:" + *it);
			break;
		}
	}
#endif // PEKWM_HAVE_DBUS
...
PekwmPanel::addWidgets(void)
{
	...
	for (; it != _cfg.widgetsEnd(); ++it) {
		...
		if (widget->isRequiredSize()) {
			pekwm::observerMapping()->addObserver(widget, this, 50);
		}
	}
}
...
PekwmPanel::resizeWidgets(bool scale_changed)
{
	...
	for (; it != _widgets.end(); ++it) {
		...
		case WIDGET_UNIT_REQUIRED:
			width = (*it)->getRequiredSize();
			width_left -= width;
			(*it)->setWidth(width);
			break;
		case WIDGET_UNIT_REQUIRED_MAX:
			width = (*it)->getRequiredSize(
					(*it)->getSizeReq().getSize());
			width_left -= width;
			(*it)->setWidth(width);
			break;
