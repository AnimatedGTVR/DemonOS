...
uint
TextWidget::getRequiredSize(uint max)
{
	PFont *font = _theme.getFont(CLIENT_STATE_UNFOCUSED);
	std::string text(" " + _text + " ");
	uint width = font->getWidth(text);
	if (max > 0 && width > max) {
		StringView trimmed = font->trim(text,
						PFont::FONT_TRIM_MIDDLE,
						max);
		_trimmed = trimmed.str();
		return max;
	}
	_trimmed = "";
	return width;
...
TextWidget::render(Render &rend, PSurface *surface)
{
	...
	PFont *font = _theme.getFont(CLIENT_STATE_UNFOCUSED);
	formatText();
	renderText(rend, surface, font, getX(),
		   _trimmed.empty() ? _text : _trimmed, getWidth());
}
