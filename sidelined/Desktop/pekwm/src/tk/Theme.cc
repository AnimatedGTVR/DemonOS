		  ...
		  int& pad_down)
{
	...
	if (font->useAscentDescent()) {
		...
		pad_down =
			static_cast<int>(pad / (height / font->getDescent()));
	} else {
...
static const char *border_to_string[BORDER_NO] =
	{"TOPLEFT", "TOPRIGHT",
...
int
Theme::PDecorData::getPad(Pad pad) const
{
...
Theme::load(const std::string &dir, const std::string &variant, bool force)
{
	...
	_ih->path_push_back(DATADIR "/pekwm/icons/");
