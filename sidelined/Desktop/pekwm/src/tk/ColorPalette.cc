...
static void
getSteps(ColorPaletteMode mode, std::vector<int> &steps)
{
	if (mode >= 0 && mode < COLOR_PALETTE_MODE_NO) {
 ...
 */
bool
getColors(ColorPaletteMode mode, BaseColor base, uint intensity,
...
{
	if (mode >= COLOR_PALETTE_MODE_NO || base >= BASE_COLOR_NO
