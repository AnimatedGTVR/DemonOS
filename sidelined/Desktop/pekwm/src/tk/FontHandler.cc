...
FontHandler::getFont(const std::string &font)
{
	...
	tok.erase(tok.begin());
	if (type == FONT_TYPE_X11 || type == FONT_TYPE_XMB) {
		replaceFontCharset(font_str);
	}
	PFont::Descr descr(font_str, type != FONT_TYPE_AUTO);
	pfont->load(descr);
...
FontHandler::newFont(const std::string &font,
		     std::vector<std::string> &tok, FontType &type)
{
	...
	if (Util::splitString(font, tok, "#", 0, true) > 1) {
		...
		if (tok_it->size() == 0) {
			...
			type = pekwm::str_to_font_type(*tok_it);
		}

		if (type == FONT_TYPE_NO) {
			type = FONT_TYPE_AUTO;
		} else {
			tok.erase(tok_it);
		}
...
PFont*
FontHandler::newFontX11(FontType &type)
{
...
{
	uint justify = pekwm::str_to_font_justify(str);
