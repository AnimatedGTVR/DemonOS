...
static bool
loadPalette(ColorPaletteMode mode, BaseColor base_color,
	    uint intensity, float brightness, const std::string &name,
	    std::map<std::string, std::string> &vars)
{
	std::vector<std::string> colors;
	if (! ColorPalette::getColors(mode, base_color, intensity, brightness,
				      colors)) {
		return false;
	}
	    ...
	    std::map<std::string, std::string> &vars)
{
	...
	bool err = false;
	ColorPaletteMode mode = pekwm::str_to_color_palette_mode(mode_str);
	BaseColor base_color = pekwm::str_to_base_color(base_str);
