...
#ifdef PEKWM_HAVE_XSCRNSAVER
#include <X11/extensions/scrnsaver.h>
#endif // PEKWM_HAVE_XSCRNSAVER
...
extern "C" {
	...
	handleXError(Display *dpy, XErrorEvent *ev)
	{
		...
		++xerrors_count;
		if (Debug::getLevel() >= DEBUG_LEVEL_TRACE) {
			char error_buf[256];
...
static void
_get_input_event_time(XEvent &ev, Time &t)
{
	if (ev.type == KeyPress) {
		t = ev.xkey.time;
...
X11::init(Display *dpy, bool synchronous, bool honour_randr)
{
	...
	_cursor_map[CURSOR_CROSS] = XCreateFontCursor(_dpy, XC_cross);
...
#ifdef PEKWM_HAVE_XSCRNSAVER
	{
		...
		_has_extension_xscrnsaver =
			XScreenSaverQueryExtension(_dpy, &ev_base, &err_base);
	}
#endif // PEKWM_HAVE_XSCRNSAVER
...
{
	if (_dpy) {
		if (XNextEvent(_dpy, &ev) && ! _has_extension_xscrnsaver) {
			_get_input_event_time(ev, _last_input_time);
		}
		return true;
	}
	return false;
...
X11::getWindowEvent(Window win, long event_mask, XEvent &ev)
{
	if (_dpy) {
		bool sucess =
			XWindowEvent(_dpy, win, event_mask, &ev) == Success;
		if (sucess && !_has_extension_xscrnsaver) {
			_get_input_event_time(ev, _last_input_time);
		}
		return sucess;
	}
...
bool
X11::grabKeyboard(Window win, bool owner_events)
{
	P_TRACE("grabbing keyboard");
	if (XGrabKeyboard(_dpy, win, owner_events, GrabModeAsync, GrabModeAsync,
		 ...
		 bool owner_events)
{
	...
	if (XGrabPointer(_dpy, win, owner_events, event_mask,
...
/**
 * Get the current time of the server by updating _PEKWM_QUERY_TIME atom on
 * the root and reading the PropertyNotify event after.
 */
long
X11::queryTime(bool select_input)
{
	if (select_input) {
		selectInput(_root, PropertyChangeMask);
	}

	Cardinal query = time(nullptr);
	long server_time = 0;
	if (_dpy && setCardinal(_root, PEKWM_QUERY_TIME, query)) {
		XEvent ev = {};
		getNextEvent(ev);
		if (ev.type == PropertyNotify) {
			server_time = ev.xproperty.time;
		}
	}
...
/**
 * Return idle time in seconds, use the XSS extension if available, else
 * the last keyboard/mouse event time vs current time.
 */
long
X11::getIdleTime()
{
	...
	if (XScreenSaverQueryInfo(_dpy, _root, &info)) {
		return info.idle / 1000;
	}
#endif // PEKWM_HAVE_XSCRNSAVER
       Time cur = queryTime(false);
       if (_last_input_time != 0 && cur > _last_input_time) {
	       return (cur - _last_input_time) / 1000;
       }
       return 0;
}
...
X11::getScreenChangeNotification(XEvent *ev, ScreenChangeNotification &scn)
{
	...
	if (_honour_randr
		...
		updateGeometry();
		scn.width = _screen_gm.width;
		scn.height = _screen_gm.height;
		return true;
	}
#endif // PEKWM_HAVE_XRANDR
	return false;
...
int
X11::getNearestHead(int x, int y, Direction dx, Direction dy)
{
	if (_heads.size() < 2) {
		return -1;
	}
...
{
	return changeProperty(win, _atoms[aname], format, 32, PropModeReplace,
			      reinterpret_cast<uchar*>(&value), 1);
}
...
void
X11::drawRectangle(Drawable d, GC gc, const Geometry &gm)
{
	if (_dpy && gm.width > 0 && gm.height > 0) {
		XDrawRectangle(_dpy, d, gc, gm.x, gm.y, gm.width, gm.height);
	}
}
...
{
	KeySym keysym;
	if (X11_Xkb_KeycodeToKeysym(keycode, keysym)) {
		return keysym;
	}
...
bool X11::_has_extension_xscrnsaver = false;
