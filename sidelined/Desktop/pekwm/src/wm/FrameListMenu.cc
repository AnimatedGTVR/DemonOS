...
FrameListMenu::handleItemExec(PMenu::Item *item)
{
	...
	switch (_menu_type) {
	case MENU_TYPE_GOTO:
	case MENU_TYPE_GOTO_CLIENT:
		handleGotomenu(item_client);
		break;
	case MENU_TYPE_ICON:
		handleIconmenu(item_client);
		break;
	case MENU_TYPE_ATTACH_CLIENT:
	case MENU_TYPE_ATTACH_FRAME:
		handleAttach(wo_ref_client, item_client,
			     (_menu_type == MENU_TYPE_ATTACH_FRAME));
		break;
	case MENU_TYPE_ATTACH_CLIENT_IN_FRAME:
	case MENU_TYPE_ATTACH_FRAME_IN_FRAME:
		handleAttach(item_client, wo_ref_client,
			     (_menu_type == MENU_TYPE_ATTACH_FRAME_IN_FRAME));
		break;
	default:
		// do nothing
		break;
...
FrameListMenu::updateFrameListMenu(void)
{
	...
	if (_menu_type == MENU_TYPE_ATTACH_CLIENT
	    || _menu_type == MENU_TYPE_GOTO_CLIENT) {
		show_clients = true;
	} else if (_menu_type == MENU_TYPE_ICON) {
			       ...
			       bool insert_separator)
{
	...
	for (; it != frame->end(); ++it) {
		...
		insert(name, false, ae, *it,
		       static_cast<Client*>(*it)->getIcon());
	}

	// add separator
	if (insert_separator) {
		PMenu::Item *item = new PMenu::Item("", false);
