...
FocusToggleEventHandler::stop(void)
{
	...
	if (_fo_wo) {
		...
		if (_fo_wo->isIconified()) {
			...
			_fo_wo->mapWindow();
			_fo_wo->raise();
		} else if (_raise == RAISE_END_RAISE) {
			_fo_wo->raise();
		}

		// Give focus
		Workspaces::giveInputFocus(_fo_wo, true);
	}
...
FocusToggleEventHandler::setFocusedWo(PWinObj *fo_wo)
{
	...
	if (_fo_wo) {
		...
		if (_raise == RAISE_ALWAYS_RAISE) {
			...
			_fo_wo->raise();
		} else if (_raise == RAISE_TEMP_RAISE) {
