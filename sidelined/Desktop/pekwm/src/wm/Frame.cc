...
ActionEvent Frame::_ae_move = ActionEvent(1, ACTION_TYPE_MOVE);
ActionEvent Frame::_ae_resize = ActionEvent(1, ACTION_TYPE_RESIZE);
ActionEvent Frame::_ae_move_resize = ActionEvent(1, ACTION_TYPE_MOVE_RESIZE);
	  ...
	  _non_fullscreen_layer(LAYER_NORMAL)
{
	...
	if (ap) {
		if (ap->isMask(PROPERTY_TYPE_FRAME_GEOMETRY|
			       PROPERTY_TYPE_CLIENT_GEOMETRY)) {
			ap_geometry = true;
		...
		if (ap->isMask(PROPERTY_TYPE_PLACE_NEW)) {
			place = ap->place_new;
		}
	}
...
Frame::handleLeaveEvent(XCrossingEvent *ev)
{
	...
	MouseActionList ln = MOUSE_ACTION_LIST_TITLE_FRAME;
...
Frame::getState(Client *client)
{
	...
	if (isFullscreen() != client->isFullscreen()) {
...
Frame::applyState(Client *client)
{
	...
	client->getState().maximized_horz = isMaximizedHorz();
	client->getState().maximized_vert = isMaximizedVert();
...
Frame::setupAPGeometry(Client *client, AutoProperty *ap)
{
	...
	// get client geometry
	if (ap->isMask(PROPERTY_TYPE_CLIENT_GEOMETRY)) {
...
Frame::clearFillStateAfterResize()
{
	if (isAnyFill()) {
		clearFill();
		_client->getState().fullscreen = false;
...
Frame::setStateFullscreen(StateAction sa)
{
	...
	if (isFullscreen()) {
		bool state_has_border =
			(_non_fullscreen_decor_state&DECOR_STATE_BORDER)
			== DECOR_STATE_BORDER;
		if (state_has_border != hasBorder()) {
			setBorder(STATE_TOGGLE);
		}
		bool state_has_titlebar =
			(_non_fullscreen_decor_state&DECOR_STATE_TITLEBAR)
			== DECOR_STATE_TITLEBAR;
...
{
	if (! ActionUtil::needToggle(sa, isSkip(skip))) {
		return;
	}

	if (isSkip(skip)) {
		setSkip(getSkip() & ~skip);
	} else {
		setSkip(getSkip() | skip);
	}
}
...
Frame::readAutoprops(ApplyOn type)
{
	...
	if (data->isMask(PROPERTY_TYPE_STICKY) && _sticky != data->sticky) {
		toggleSticky();
	}
	if (data->isMask(PROPERTY_TYPE_SHADED)
	    && (isShaded() != data->shaded)) {
		setShaded(STATE_UNSET);
	}
	if (data->isMask(PROPERTY_TYPE_MAXIMIZED_HORIZONTAL)
	    && (isMaximizedHorz() != data->maximized_horizontal)) {
		setStateMaximized(STATE_TOGGLE, true, false, false);
	}
	if (data->isMask(PROPERTY_TYPE_MAXIMIZED_VERTICAL)
	    && (isMaximizedVert() != data->maximized_vertical)) {
		setStateMaximized(STATE_TOGGLE, false, true, false);
	}
	if (data->isMask(PROPERTY_TYPE_FULLSCREEN)
	    && (isFullscreen() != data->fullscreen)) {
		setStateFullscreen(STATE_TOGGLE);
	}

	if (data->isMask(PROPERTY_TYPE_ICONIFIED)
	...
	if (data->isMask(PROPERTY_TYPE_WORKSPACE)) {
	...
	if (data->isMask(PROPERTY_TYPE_LAYER)
	    && (data->layer <= LAYER_MENU)) {
		_client->setLayer(data->layer);
		raise(); // restack the frame
	}

	if (data->isMask(PROPERTY_TYPE_FRAME_GEOMETRY|
			 PROPERTY_TYPE_CLIENT_GEOMETRY)) {
	...
	if (data->isMask(PROPERTY_TYPE_BORDER)
	    && (hasBorder() != data->border)) {
		setStateDecorBorder(STATE_TOGGLE);
	}
	if (data->isMask(PROPERTY_TYPE_TITLEBAR)
	    && (hasTitlebar() != data->titlebar)) {
		setStateDecorTitlebar(STATE_TOGGLE);
	}

	if (data->isMask(PROPERTY_TYPE_SKIP)) {
		_client->setSkip(data->skip);
		setSkip(_client->getSkip());
	}

	if (data->isMask(PROPERTY_TYPE_FOCUSABLE)) {
...
Frame::handleClientMessage(XClientMessageEvent *ev, Client *client)
{
	...
	if (ev->message_type == X11::getAtom(STATE)) {
		...
		handleClientMessageNetActiveWindow(ev, client);
...
void
Frame::handleClientMessageNetActiveWindow(XClientMessageEvent *ev,
					  Client *client)
{
	...
	Window focus_win = PWinObj::getFocusedWindow();
	Cardinal focus_time;
	if (ev->data.l[0] == NET_WM_SOURCE_CLIENT
	    && pekwm::config()->isFocusStealProtect()
	    && X11::getCardinal(focus_win, NET_WM_USER_TIME, focus_time)
	    && ev->data.l[1] < focus_time) {
		// The message is not from a pager and focus steal protection
		// is enabled, check times
		P_TRACE(client->getWindow() <<
			" _NET_ACTIVE_WINDOW time set to " << ev->data.l[1] <<
			", less than focused window " << focus_win <<
			" time " << focus_time << ", not giving input focus");
			return;
	}
	...
	// If window is not mapped, ensure the correct workspace is active
	// and map the window.
	if (! _mapped) {
		if (_workspace != Workspaces::getActive() && !isSticky()) {
