...
ActionHandler::handleWoAction(const ActionPerformed *ap, ActionEvent::it it)
{
	...
	case ACTION_TYPE_FOCUS_DIRECTIONAL:
		actionFocusDirectional(ap->wo, Direction(it->getParamI(0)),
				       it->getParamI(1));
		break;
				 ...
				 Client* client, Frame* frame)
{
	...
	case ACTION_TYPE_GROUPING_DRAG:
		actionGroupingDrag(ap, frame, client,
				   it->getParamI(0));
		break;
	case ACTION_TYPE_ACTIVATE_CLIENT:
	...
	case ACTION_TYPE_RESIZE:
		actionResize(ap, frame, client,
			     static_cast<Border>(
				     it->getParamI(0, BORDER_NO)));
		break;
	case ACTION_TYPE_MOVE_RESIZE:
		actionMoveResize(frame);
		break;
	case ACTION_TYPE_CLOSE:
		client->close();
		break;
	case ACTION_TYPE_CLOSE_FRAME:
		frame->close();
		break;
	...
	case ACTION_TYPE_LOWER:
		...
		break;
	case ACTION_TYPE_ACTIVATE_OR_RAISE:
		if ((ap->type == ButtonPress || ap->type == ButtonRelease)
	...
	case ACTION_TYPE_MOVE_TO_EDGE:
		actionMoveToEdge(frame, Edge(it->getParamI(0)));
		break;
	case ACTION_TYPE_FILL_EDGE:
		actionFillEdge(frame, Edge(it->getParamI(0)), it->getParamI(1));
		break;
	...
	case ACTION_TYPE_DETACH_SPLIT_VERT:
		...
		break;
	case ACTION_TYPE_ATTACH_MARKED:
		attachMarked(frame);
		break;
	case ACTION_TYPE_ATTACH_CLIENT_IN_NEXT_FRAME:
		attachInNextPrevFrame(client, false, true);
		break;
	case ACTION_TYPE_ATTACH_CLIENT_IN_PREV_FRAME:
		attachInNextPrevFrame(client, false, false);
		break;
	case ACTION_TYPE_ATTACH_FRAME_IN_NEXT_FRAME:
		attachInNextPrevFrame(client, true, true);
		break;
	case ACTION_TYPE_ATTACH_FRAME_IN_PREV_FRAME:
				...
				PMenu* menu)
{
	switch (it->getAction()) {
		// menu navigation
	case ACTION_TYPE_MENU_NEXT:
		menu->selectNextItem();
		break;
	case ACTION_TYPE_MENU_PREV:
		menu->selectPrevItem();
		break;
	case ACTION_TYPE_MENU_GOTO:
		menu->selectItemNum(it->getParamI(0));
		break;
	case ACTION_TYPE_MENU_SELECT:
	case ACTION_TYPE_MENU_ENTER_SUBMENU:
		if (menu->getItemCurr()
		...
		} else if (it->getAction() == ACTION_TYPE_MENU_SELECT) {
		...
		break;
	case ACTION_TYPE_MENU_LEAVE_SUBMENU:
		menu->gotoParentMenu();
		break;
	case ACTION_TYPE_CLOSE:
		menu->unmapAll();
			       ...
			       Client* client, Frame* frame)
{
	...
	case ACTION_TYPE_PREV_FRAME_MRU: {
		bool mru = it->getAction() == ACTION_TYPE_NEXT_FRAME_MRU
			|| it->getAction() == ACTION_TYPE_PREV_FRAME_MRU;
		int dir = (it->getAction() == ACTION_TYPE_NEXT_FRAME
			   || it->getAction() == ACTION_TYPE_NEXT_FRAME_MRU)
			? 1 : -1;
		EventHandler *event_handler =
	...
	case ACTION_TYPE_SHOW_MENU:
		...
		break;
	case ACTION_TYPE_HIDE_ALL_MENUS:
	...
	case ACTION_TYPE_WARP_POINTER:
		actionWarpPointer(it->getParamI(0),
				  it->getParamI(1));
		break;
	case ACTION_TYPE_HIDE_WORKSPACE_INDICATOR:
...
ActionEvent*
ActionHandler::findMouseAction(uint button, uint state, MouseEvent type,
 ...
 */
bool
ActionHandler::actionMoveToEdge(PWinObj *wo, Edge edge)
{
	...
	case EDGE_BOTTOM_RIGHT:
		...
		break;
	case EDGE_BOTTOM_EDGE:
		x = wo->getX();
		y = head.y + head.height - wo->getHeight();
		break;
	case EDGE_BOTTOM_CENTER_EDGE:
		x = real_head.x + ((real_head.width - wo->getWidth()) / 2);
		y = head.y + head.height - wo->getHeight();
		break;
	...
	case EDGE_LEFT_EDGE:
		...
		y = wo->getY();
		break;
	case EDGE_LEFT_CENTER_EDGE:
		x = head.x;
		y = real_head.y + ((real_head.height - wo->getHeight()) / 2);
		break;
	case EDGE_RIGHT_EDGE:
		x = head.x + head.width - wo->getWidth();
		y = wo->getY();
		break;
	case EDGE_RIGHT_CENTER_EDGE:
		x = head.x + head.width - wo->getWidth();
		y = real_head.y + ((real_head.height - wo->getHeight()) / 2);
		break;
	case EDGE_CENTER:
 ...
 */
bool
ActionHandler::actionFillEdge(Frame *frame, Edge edge, int percent)
{
	...
	Edge old_edge = frame->getEdgeFilled();
	if (old_edge != EDGE_NO && isEdgeOpposites(old_edge, edge)) {
...
bool
ActionHandler::fillEdgeGeometry(const Geometry &head, Edge edge, int percent,
				Geometry &gm)
{
	if (percent < 1) {
		percent = 1;
...
bool
ActionHandler::isEdgeOpposites(Edge lhs, Edge rhs)
{
...
ActionHandler::actionResize(const ActionPerformed *ap,
			    Frame *frame, Client *client, Border pos)
{
	bool resize = true;

	bool left = false, top = false, x = false, y = false;
	if (ap->type == MotionNotify && pos == BORDER_NO) {
...
ActionHandler::actionSendToWorkspace(PDecor *decor, bool focus, int direction)
{
	...
	switch (static_cast<WorkspaceChange>(direction)) {
	case WORKSPACE_CHANGE_LEFT:
	case WORKSPACE_CHANGE_PREV:
		if (cur_act > row_min) {
			new_workspace = cur_act - 1;
		} else if (udirection == WORKSPACE_CHANGE_PREV) {
			new_workspace = row_max;
		}
		break;
	case WORKSPACE_CHANGE_NEXT:
	case WORKSPACE_CHANGE_RIGHT:
		if (cur_act < row_max) {
			new_workspace = cur_act + 1;
		} else if (udirection == WORKSPACE_CHANGE_NEXT) {
			new_workspace = row_min;
		}
		break;
	case WORKSPACE_CHANGE_PREV_V:
	case WORKSPACE_CHANGE_UP:
		if (cur_act >= per_row) {
			new_workspace = cur_act - per_row;
		} else if (udirection == WORKSPACE_CHANGE_PREV_V) {
			// Bottom left + column
			new_workspace = Workspaces::size() - per_row
				+ cur_act - cur_row * per_row;
		}
		break;
	case WORKSPACE_CHANGE_NEXT_V:
	case WORKSPACE_CHANGE_DOWN:
		if ((cur_act + per_row) < Workspaces::size()) {
			new_workspace = cur_act + per_row;
		} else if (udirection == WORKSPACE_CHANGE_NEXT_V) {
			new_workspace = cur_act - cur_row * per_row;
		}
		break;
	case WORKSPACE_CHANGE_LAST:
				     ...
				     uint direction)
{
	...
	// actually did move
	if (Workspaces::gotoWorkspace(Direction(direction), false, true)) {
		int x, y;
...
void
ActionHandler::actionFocusDirectional(PWinObj *wo, Direction dir, bool raise)
{
	PWinObj *wo_focus =
			      ...
			      uint e_type, PWinObj *wo_ref)
{
	...
	if (menu->isMapped()) {
		...
		if (wo_ref_menu
		    ...
		    // title
		    && (wo_ref_menu->getMenuType() != MENU_TYPE_ROOT)
		    && (wo_ref_menu->getMenuType() != MENU_TYPE_ROOT_STANDALONE)
		   ) {
					   ...
					   float new_aspect)
{
	...
	int old_height =
		static_cast<int>((1.0 - new_aspect) * oldf->getHeight());
