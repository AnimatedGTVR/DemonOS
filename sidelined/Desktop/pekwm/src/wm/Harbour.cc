...
Harbour::removeDockApp(DockApp *da)
{
	...
	if (it != _dapps.end()) {
		_dapps.erase(it);
		// remove the dockapp to the stacking list
		Workspaces::remove(da);
		delete da;
...
{
	std::vector<DockApp*>::iterator it = _dapps.begin();
...
Harbour::updateHarbourSize(void)
{
	...
	for (; it != _dapps.end(); ++it) {
		switch (_cfg->getHarbourPlacement())
			{
			case HARBOUR_PLACEMENT_TOP:
			case HARBOUR_PLACEMENT_BOTTOM:
				if ((*it)->getHeight() > _size)
					_size = (*it)->getHeight();
				break;
			case HARBOUR_PLACEMENT_LEFT:
			case HARBOUR_PLACEMENT_RIGHT:
...
Harbour::placeDockApp(DockApp *da)
{
	...
	bool right =
		(_cfg->getHarbourOrientation() == ORIENTATION_BOTTOM_TO_TOP);
...
Harbour::placeDockAppInsideScreen(DockApp *da)
{
	...
	if ((pos == HARBOUR_PLACEMENT_TOP)
	    || (pos == HARBOUR_PLACEMENT_BOTTOM)) {
		// check horizontal position
		if (da->getX() < head.x) {
			da->move(head.x, da->getY());
		} else if (da->getRX()
			   > static_cast<signed>(head.x + head.width)) {
			da->move(head.x + head.width - da->getWidth(),
				 da->getY());
		}

		// check vertical position
		if ((pos == HARBOUR_PLACEMENT_TOP) && (da->getY() != head.y)) {
			da->move(da->getX(), head.y);
		} else if ((pos == HARBOUR_PLACEMENT_BOTTOM)
			   && (da->getBY()
			       != static_cast<signed>(head.y + head.height))) {
			da->move(da->getX(),
				 head.y + head.height - da->getHeight());
		}
		...
		// check horizontal position
		if ((pos == HARBOUR_PLACEMENT_LEFT) && (da->getX() != head.x)) {
			da->move(head.x, da->getY());
		} else if ((pos == HARBOUR_PLACEMENT_RIGHT)
			   && (da->getRX()
			       != static_cast<signed>(head.x + head.width))) {
			da->move(head.x + head.width - da->getWidth(),
				 da->getY());
...
Harbour::insertDockAppSorted(DockApp *da)
{
	...
	if (da->getPosition() == 0) {
		...
		for (; it != _dapps.end()
			...
			;
		for (; it != _dapps.end()
		       && da->getPosition() >= (*it)->getPosition(); ++it)
			;
	}
