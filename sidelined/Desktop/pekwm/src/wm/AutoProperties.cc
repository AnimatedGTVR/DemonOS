...
static Util::StringTo<AtomName> window_type_map[] = // subset of enum
				     ...
				     Property *prop)
{
	...
	if (Util::splitString(apply_on, tokens, " \t", 5)) {
		...
		for (; it != tokens.end(); ++it) {
			int num = pekwm::str_to_apply_on(*it);
			       ...
			       AutoProperty* property)
{
	...
	for (; it != section->end(); ++it) {
		PropertyGroup property_group =
			pekwm::str_to_property_group((*it)->getName());
		switch (property_group) {
		case PROPERTY_GROUP_SIZE:
			property->group_size = std::stoi((*it)->getValue());
			break;
		case PROPERTY_GROUP_BEHIND:
			property->group_behind =
				Util::isTrue((*it)->getValue());
			break;
		case PROPERTY_GROUP_FOCUSED_FIRST:
			property->group_focused_first =
				Util::isTrue((*it)->getValue());
			break;
		case PROPERTY_GROUP_GLOBAL:
			property->group_global =
				Util::isTrue((*it)->getValue());
			break;
		case PROPERTY_GROUP_RAISE:
...
_mk_menu_property()
{
	...
	prop->maskAdd(PROPERTY_TYPE_TITLEBAR);
	prop->titlebar = false;
	prop->maskAdd(PROPERTY_TYPE_BORDER);
	prop->border = false;
	prop->maskAdd(PROPERTY_TYPE_SKIP);
...
AutoProperties::setDefaultTypeProperties(void)
{
	...
	if (! findWindowTypeProperty(WINDOW_TYPE_DESKTOP)) {
		AutoProperty *prop = new AutoProperty();
		prop->maskAdd(PROPERTY_TYPE_CLIENT_GEOMETRY);
		prop->client_gm_mask =
			X11::parseGeometry("0x0+0+0", prop->client_gm);
		prop->maskAdd(PROPERTY_TYPE_STICKY);
		prop->sticky = true;
		prop->maskAdd(PROPERTY_TYPE_TITLEBAR);
		prop->titlebar = false;
		prop->maskAdd(PROPERTY_TYPE_BORDER);
		...
		prop->skip =
			...
			|SKIP_TASKBAR;
		prop->maskAdd(PROPERTY_TYPE_LAYER);
		prop->layer = LAYER_DESKTOP;
		prop->maskAdd(PROPERTY_TYPE_FOCUSABLE);
		prop->focusable = false;
		prop->maskAdd(PROPERTY_TYPE_DISALLOWED_ACTIONS);
	...
	if (! findWindowTypeProperty(WINDOW_TYPE_MENU)) {
		_window_type_prop_map[WINDOW_TYPE_MENU] = _mk_menu_property();
	}
	...
	if (! findWindowTypeProperty(WINDOW_TYPE_DROPDOWN_MENU)) {
		_window_type_prop_map[WINDOW_TYPE_DROPDOWN_MENU] =
			_mk_menu_property();
	}

	// POPUPMENU
	if (! findWindowTypeProperty(WINDOW_TYPE_POPUP_MENU)) {
		_window_type_prop_map[WINDOW_TYPE_POPUP_MENU] =
			_mk_menu_property();
	}
	...
	if (! findWindowTypeProperty(WINDOW_TYPE_TOOLTIP)) {
		...
		prop->maskAdd(PROPERTY_TYPE_FOCUS_NEW);
		prop->skip = SKIP_MENUS|SKIP_FOCUS_TOGGLE|SKIP_SNAP;
		prop->maskAdd(PROPERTY_TYPE_SKIP);
...
AutoProperties::parseAutoPropertyType(CfgParser::Entry* it, AutoProperty* prop)
{
	...
	PropertyType property_type =
		pekwm::str_to_property_type(it->getName());
	switch (property_type) {
	case PROPERTY_TYPE_STICKY:
		prop->maskAdd(PROPERTY_TYPE_STICKY);
		prop->sticky = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_SHADED:
		prop->maskAdd(PROPERTY_TYPE_SHADED);
		prop->shaded = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_MAXIMIZED_VERTICAL:
		prop->maskAdd(PROPERTY_TYPE_MAXIMIZED_VERTICAL);
		prop->maximized_vertical = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_MAXIMIZED_HORIZONTAL:
		prop->maskAdd(PROPERTY_TYPE_MAXIMIZED_HORIZONTAL);
		prop->maximized_horizontal = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_ICONIFIED:
		prop->maskAdd(PROPERTY_TYPE_ICONIFIED);
		prop->iconified = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_BORDER:
		prop->maskAdd(PROPERTY_TYPE_BORDER);
		prop->border = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_TITLEBAR:
		prop->maskAdd(PROPERTY_TYPE_TITLEBAR);
		prop->titlebar = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_FRAME_GEOMETRY:
		prop->maskAdd(PROPERTY_TYPE_FRAME_GEOMETRY);
		prop->frame_gm_mask =
			X11::parseGeometry(it->getValue(), prop->frame_gm);
		break;
	case PROPERTY_TYPE_CLIENT_GEOMETRY:
		prop->maskAdd(PROPERTY_TYPE_CLIENT_GEOMETRY);
		prop->client_gm_mask =
			X11::parseGeometry(it->getValue(), prop->client_gm);
		break;
	case PROPERTY_TYPE_LAYER:
		prop->layer = pekwm::str_to_layer(it->getValue());
		if (prop->layer != LAYER_NONE) {
			prop->maskAdd(PROPERTY_TYPE_LAYER);
		}
		break;
	case PROPERTY_TYPE_WORKSPACE:
		try {
			prop->workspace = std::stoi(it->getValue()) - 1;
			prop->maskAdd(PROPERTY_TYPE_WORKSPACE);
		} catch (std::invalid_argument&) {
		}
		break;
	case PROPERTY_TYPE_SKIP:
		prop->maskAdd(PROPERTY_TYPE_SKIP);
		tokens.clear();
		if ((Util::splitString(it->getValue(), tokens, " \t"))) {
			for (token_it = tokens.begin();
			     token_it != tokens.end();
			     ++token_it) {
				prop->skip |= pekwm::str_to_skip(*token_it);
			}
		}
		break;
	case PROPERTY_TYPE_FULLSCREEN:
		prop->maskAdd(PROPERTY_TYPE_FULLSCREEN);
		prop->fullscreen = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_PLACE_NEW:
		prop->maskAdd(PROPERTY_TYPE_PLACE_NEW);
		prop->place_new = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_FOCUS_NEW:
		prop->maskAdd(PROPERTY_TYPE_FOCUS_NEW);
		prop->focus_new = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_FOCUSABLE:
		prop->maskAdd(PROPERTY_TYPE_FOCUSABLE);
		prop->focusable = Util::isTrue(it->getValue());
		break;
	case PROPERTY_TYPE_CFG_DENY:
		prop->maskAdd(PROPERTY_TYPE_CFG_DENY);
		tokens.clear();
		if ((Util::splitString(it->getValue(), tokens, " \t"))) {
			for (token_it = tokens.begin();
			     token_it != tokens.end();
			     ++token_it) {
				prop->cfg_deny |= pekwm::str_to_cfg_deny(
						*token_it);
			}
		}
		break;
	case PROPERTY_TYPE_ALLOWED_ACTIONS:
		prop->maskAdd(PROPERTY_TYPE_ALLOWED_ACTIONS);
		pekwm::config()->parseActionAccess(
				it->getValue(),
				prop->allowed_actions);
		break;
	case PROPERTY_TYPE_DISALLOWED_ACTIONS:
		prop->maskAdd(PROPERTY_TYPE_DISALLOWED_ACTIONS);
		pekwm::config()->parseActionAccess(
				it->getValue(),
				prop->disallowed_actions);
		break;
	case PROPERTY_TYPE_OPACITY:
		prop->maskAdd(PROPERTY_TYPE_OPACITY);
		Config::parseOpacity(it->getValue(),
				     prop->focus_opacity,
				     prop->unfocus_opacity);
		break;
	case PROPERTY_TYPE_DECOR:
		prop->maskAdd(PROPERTY_TYPE_DECOR);
	...
	case PROPERTY_TYPE_ICON: {
		...
		if (image == nullptr) {
			...
			prop->maskAdd(PROPERTY_TYPE_ICON);
			prop->icon = new PImageIcon(image);
			_image_handler->returnImage(image);
		}
		break;
	}
	case PROPERTY_TYPE_PLACEMENT:
		prop->maskAdd(PROPERTY_TYPE_PLACEMENT);
