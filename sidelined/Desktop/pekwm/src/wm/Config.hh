...
{
public:
	typedef std::map<MouseActionList, std::vector<ActionEvent>* >
		mouse_action_map;
	...
	DebugLevel getDebugLevel() const { return _debug_level; }
	...
	int getScreenEdgeSize(ScreenEdge edge) const {
	...
	bool isFocusStealProtect() const {
		return _screen_focus_steal_protect > 0;
	}
	...
	std::vector<ActionEvent> *getMouseActionList(MouseActionList mal) {
		return _mouse_action_map[mal];
	}
	...
	// map parsing
	bool parseActionAccess(const std::string &action_mask_string,
			       uint &mask);
