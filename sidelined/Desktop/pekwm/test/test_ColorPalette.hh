...
TestColorPalette::testGetColorsAsStrings()
{
	...
	res = ColorPalette::getColors(COLOR_PALETTE_MODE_COMPLEMENTARY,
				      BASE_COLOR_RED, 0, 1.0,
				      strs);
	...
	strs.clear();
	res = ColorPalette::getColors(COLOR_PALETTE_MODE_TETRAD,
				      BASE_COLOR_ORANGE, 4,
				      1.0, strs);
	...
	strs.clear();
	res = ColorPalette::getColors(COLOR_PALETTE_MODE_SINGLE,
				      BASE_COLOR_PURPLE, 0,
				      1.0, strs);
...
TestColorPalette::testGetColorsInputValidation()
{
	bool res;
	std::vector<std::string> strs;
	res = ColorPalette::getColors(COLOR_PALETTE_MODE_NO,
				      BASE_COLOR_RED, 0, 1.0,
				      strs);
	ASSERT_FALSE("invalid mode", res);
	ASSERT_EQUAL("invalid mode", 0, strs.size());

	res = ColorPalette::getColors(COLOR_PALETTE_MODE_TRIAD,
				      BASE_COLOR_NO, 0, 1.0,
				      strs);
