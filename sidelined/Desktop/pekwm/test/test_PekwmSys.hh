...
private:
	...
	static void testGetTimeoutIdleCheck();
};
...
TestPekwmSys::run_test(TestSpec spec, bool status)
{
	...
	TEST_FN(spec, "getTimeoutIdleCheck", testGetTimeoutIdleCheck());
	return status;
}
...
void
TestPekwmSys::testGetTimeoutIdleCheck()
{
	OsMock os;
	PekwmSys sys("", false, &os);

	ASSERT_EQUAL("no idle check", PEKWM_SYS_CHECK_IDLE_TIME_INTERVAL_MS,
		     sys.getTimeoutIdleCheck());

	sys._cfg.setScreenLockIdleTimeS(30);
	ASSERT_EQUAL("idle < interval", 30000, sys.getTimeoutIdleCheck());

	sys._screen_lock_pid = 1;
	ASSERT_EQUAL("screen locked", PEKWM_SYS_CHECK_IDLE_TIME_INTERVAL_MS,
		     sys.getTimeoutIdleCheck());
}
