...
TestActionHandler::testMoveToEdge()
{
	...
	ActionHandler::actionMoveToEdge(&wo, EDGE_RIGHT_CENTER_EDGE);
	ASSERT_EQUAL("EDGE_RIGHT_CENTER_EDGE", Geometry(700, 250, 100, 100),
		     wo.getGeometry());

	// head 1
	wo.moveResize(800, 0, 100, 100);
	ActionHandler::actionMoveToEdge(&wo, EDGE_RIGHT_CENTER_EDGE);
	ASSERT_EQUAL("EDGE_RIGHT_CENTER_EDGE", Geometry(1500, 250, 100, 100),
		     wo.getGeometry());
}
...
TestActionHandler::testFillEdgeGeometry()
{
	...
	ActionHandler::fillEdgeGeometry(head, EDGE_LEFT_EDGE, 50, gm);
	ASSERT_EQUAL("EDGE_LEFT_EDGE", Geometry(0, 0, 100, 100), gm);

	ActionHandler::fillEdgeGeometry(head, EDGE_RIGHT_EDGE, 50, gm);
	ASSERT_EQUAL("EDGE_RIGHT_EDGE", Geometry(100, 0, 100, 100), gm);

	ActionHandler::fillEdgeGeometry(head, EDGE_TOP_EDGE, 50, gm);
	ASSERT_EQUAL("EDGE_TOP_EDGE", Geometry(0, 0, 200, 50), gm);

	ActionHandler::fillEdgeGeometry(head, EDGE_BOTTOM_EDGE, 50, gm);
	ASSERT_EQUAL("EDGE_BOTTOM_EDGE", Geometry(0, 50, 200, 50), gm);
}
