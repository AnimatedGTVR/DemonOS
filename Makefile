...
# Rust replacement compositor, Stage 2: a real minimal compositor that
# actually holds the DISPLAY capability, queries the real display, and
# presents one real solid-color frame -- proving the syscall path this
# stage needs before porting over user/compositor.mko's real window-
# management logic (window table, z-order, focus, damage) in later stages.
RUST_COMPOSITOR_ELF := $(BUILD)/rust-compositor.elf
MAKO_REPO := ../MAKO
...
$(RUST_COMPOSITOR_ELF): rust/compositor/src/main.rs rust/compositor/Cargo.toml rust/demon-abi/src/lib.rs rust/demon-abi/Cargo.toml rust/target-user.json rust/linker.ld rust/.cargo/config.toml | $(BUILD)
	cd rust/compositor && PATH="$(HOME)/.cargo/bin:$$PATH" cargo build -Z build-std=core -Z json-target-spec --target ../target-user.json --release
	cp rust/compositor/target/target-user/release/compositor $@
	$(STRIP) -s $@
