...
| --- | --- | --- | --- |
| `deltarune-ch1` | DELTARUNE Chapter 1 / SURVEY_PROGRAM | GameMaker bytecode 17 | Full structural probe passes against the real, legitimately-owned data.win (`BUTTERSCOTCH_GAME_PROBE_OK`: 14129 strings, 1680 code entries, 353 objects, 147 rooms, 9 textures, 151 sounds, 56 audio entries). A real room (room 54: 640x480, 16 placed instances) composites correctly into one real frame (`BUTTERSCOTCH_D4_ROOM_COMPOSITE_OK`) and is now actually presented on screen (`BUTTERSCOTCH_D5_DISPLAY_PRESENT_OK`) via the same raw display path Doom/ClassiCube use -- confirmed with a real QEMU screenshot showing the correct, recognizable scene. The bytecode scanner validates every one of the game's real CODE entries end to end (`BUTTERSCOTCH_D6_BYTECODE_SCAN_OK`: 1680/1680 entries, 419564 instructions, 35865 branches, 0 unknown opcodes). The VM now actually *executes* real compiled scripts, not just validates their shape (`BUTTERSCOTCH_D8_VM_SCRIPT_OK`: 3/3 real scripts run their entire compiled GMS2.3 preamble correctly) -- see "Real VM execution" and "Executing real user scripts" below. The general per-instance dispatch machinery the interactive live loop uses (real Step/Collision handlers resolved per real `objectId`, not a hardcoded fixture instance) has now been run against room 54's real instances headlessly, decoupled from live rendering (`BUTTERSCOTCH_D9_REAL_DISPATCH_PARTIAL`: of 16 real instances, 2 declare a real Step handler). Fixing `call_builtin`'s arity handling and adding `@@NullObject@@`/`@@Global@@`/`@@This@@`/`@@Other@@` got the player's real Step script past its first blocker; investigating what looked like a stack-underflow past that point uncovered a real, previously-silent bug: dispatch selected its target CODE entry via a `1u << codeId` bitmask, and the player's real Step handler is CODE index 761 -- `1u << 761` is undefined behavior, so the VM was silently executing an unrelated real script instead of the one actually requested (any object whose handler lands past CODE index 31 was affected, i.e. most of the game). Fixed by replacing the bitmask with an explicit CODE-id list, then closed a run of real, mechanical VM-completeness gaps real dispatch surfaced one at a time: the dedicated `OP_PUSHLOC`/`OP_PUSHGLB` local/global variable-push opcodes, the missing Int64/Float/Bool/Int16 `OP_PUSH` literal types, and `OP_CONV`'s real truthiness/narrowing semantics (confirmed against upstream's actual `RValue_toBool`, not a naive guess -- e.g. GML truthiness is `>0`/`>0.5`, not "nonzero"). One of the two real instances ran its entire real Step script to completion -- the first real Deltarune object script, not just a synthetic self-test, to execute successfully end to end. The player's own script then hit its first world-aware builtin, `place_meeting(x, y, obj)` -- needing the VM to know about *other* instances for the first time, unlike every builtin before it. Added a bounded, AABB-only world-query capability (reusing the same TPAG-rect overlap math the existing collision dispatch already uses, not a real spatial grid or precise pixel collision) and implemented `place_meeting` on top of it. Re-running the D9 probe shows the player's real script advance **over 7600 bytes further** -- `place_meeting` was gating a large chunk of real gameplay logic -- before hitting a related spatial-query builtin, `collision_rectangle`, that can reuse the same new infrastructure. 0 of the 16 instances had a resolved collision handler against another instance in this particular room. Full gameplay (movement, interaction, dialogue, live rendering against real multi-page textures, and the real builtin function surface) is not enabled yet. |
