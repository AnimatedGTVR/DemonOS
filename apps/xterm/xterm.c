...
static uint32_t visible_line(uint32_t index) {
    const uint32_t visible = scrollback_count < GRID_ROWS
        ? scrollback_count : GRID_ROWS;
...
static void terminal_key(uint32_t value, uint32_t keycode) {
    if (value == '\n') {
        terminal_execute();
        return;
    }
...
static void terminal_feed_text(Display *display, Window window, GC gc,
    ...
    uint32_t index = 0u;
    while (text[index] != '\0') {
        terminal_key((uint32_t)text[index], 0u);
        ++index;
    }
...
static void render_terminal(Display *display, Window window, GC gc) {
    ...
    for (uint32_t row = 0u; row < visible; ++row) {
        ...
        XDrawString(display, window, gc, PADDING,
                    PADDING + TITLE_HEIGHT + (int)(row * CELL_HEIGHT) + 6,
                    line, (int)length);
    }

    const int input_y = PADDING + TITLE_HEIGHT + (int)(visible * CELL_HEIGHT) + 6;
    XSetForeground(display, gc, COLOR_PROMPT);
    XDrawString(display, window, gc, PADDING, input_y, PROMPT,
                (int)PROMPT_LENGTH);
    if (input_length > 0u) {
        XSetForeground(display, gc, COLOR_TEXT);
        XDrawString(display, window, gc,
                    PADDING + (int)(PROMPT_LENGTH * CELL_WIDTH), input_y,
                    input, (int)input_length);
    }

    const int cursor_x = PADDING + (int)((PROMPT_LENGTH + input_cursor) * CELL_WIDTH);
    const int cursor_y = PADDING + TITLE_HEIGHT + (int)(visible * CELL_HEIGHT);
    XSetForeground(display, gc, COLOR_CURSOR);
    XFillRectangle(display, window, gc, cursor_x, cursor_y, CELL_WIDTH, CELL_HEIGHT);
    if (input_cursor < input_length) {
        const char cursor_char[2] = { input[input_cursor], '\0' };
        XSetForeground(display, gc, COLOR_CURSOR_CHAR);
        XDrawString(display, window, gc, cursor_x, cursor_y + 6,
                    cursor_char, 1);
    }
    XFlush(display);
}
